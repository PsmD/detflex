...
- **Movies Pagination 개선(v1)**
  ...
  - 전체 영화 페이지가 10페이지를 넘어도 10페이지까지만 보여주던 테스트 코드에 가깝던 상태에서 10페이지씩 나눠서 전체 페이지를 보여주는 로직을 구성하여 보여주도록 개선했지만 아직 페이지 그룹의 마지막 페이지 번호를 누르면 바로 다음 페이지 그룹으로 넘어가버리는 문제, 이전으로 가는 버튼 다음으로 가는 버튼의 부재 등의 문제가 있음. 빠른 시간내에 모든 문제 해결 예정
...
- **Movies Pagination 개선(v2)**
  ...
  - 맨 처음 페이지, 이전 페이지, 다음 페이지, 마지막 페이지로 갈 수 있는 버튼을 추가 했고, 1페이지라서 이전 버튼을 못누른다거나 하는 등의 그 버튼을 누를 수 없는 상황이 되면 커서 이벤트가 사라지고 색깔이 흐릿하게 변하게 구현했고 여기까지는 정상 작동. 하지만 아직 페이지 그룹의 마지막 페이지를 누르면 저절로 다음 페이지 그룹으로 넘어가는 문제는 남아있고 뒤로가기를 누르면 페이지 넘버가 변경되지 않고 화면만 이동하는 문제, 페이지 번호를 눌러놓은 상태에서 다른 메뉴로 이동하면 눌러놓은 페이지 넘버의 메뉴가 호출되는 문제가 있음(예를 들면 Now Playing 메뉴에서 8번 페이지를 눌러 놓고 Top Rating 메뉴로 이동하면 1번 페이지가 아니라 8번 페이지의 Top Rating이 나옴)
