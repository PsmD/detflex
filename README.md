...
- **Movies Pagination 개선(v1)**
  ...
  - 전체 영화 페이지가 10페이지를 넘어도 10페이지까지만 보여주던 테스트 코드에 가깝던 상태에서 10페이지씩 나눠서 전체 페이지를 보여주는 로직을 구성하여 보여주도록 개선했지만 아직 페이지 그룹의 마지막 페이지 번호를 누르면 바로 다음 페이지 그룹으로 넘어가버리는 문제, 이전으로 가는 버튼 다음으로 가는 버튼의 부재 등의 문제가 있음. 빠른 시간내에 모든 문제 개선 예정
